...
static int shiftcolsleft(double *cols, int height, int N,
                         const double *newcol)
{
...
static void rtpghi_abs(const std::complex<double> *in, int height, double *out)
{
...
static void rtpghi_phase(const std::complex<double> *in, int height, double *out)
{
    for (int ii = 0; ii < height; ++ii) {
...
static void rtpghi_fgrad(const double *phase, int M, double stretch, double *fgrad);

static void rtpghi_tgrad(const double *phase, int aanaprev, int aananext, int M,
                         double stretch, double *tgrad);

static void rtpghi_imagphase(const double *s, const double *phase, int L, std::complex<double> *c);
...
rtpghi_priv::~rtpghi_priv()
{
    delete _p;
}

double rtpghi_priv::get_stretch() const
{
    return stretch;
}
...
void rtpghi_priv::reset(const double **sinit)
{
    ...
    if (sinit) {
        for (int w = 0; w < W; ++w) {
            if (sinit[w]) {
                std::copy(sinit[w], sinit[w] + 2 * M2, _s + 2 * w * M2);
            }
        }
    }
}
                          ...
                          std::complex<double> *cout)
{
    ...
    for (int w = 0; w < W; ++w) {
        ...
        rtpghi_tgrad(phaseinCol, aanaprev, aananext, _M, _stretch, tgradCol + M2);
        ...
        rtpghi_imagphase(sCol + M2, phaseCol, M2, cout + w * M2);
    }
    ...
    : _rand(_rd()), _dist(-2.0 * M_PI, 2.0 * M_PI)
{
    ...
    _tol = tol;
    _M = M;
    _h = rtpghi_heap_init(2 * M2);
}

rtpghi_update_plan::~rtpghi_update_plan()
{
    delete _h;
}

void rtpghi_update_plan::execute_with_mask(const double *s,
                                           const double *tgrad,
                                           const double *fgrad,
                                           ...
                                           double *phase)
{
    ...
    return execute_common(p, s, tgrad, fgrad, startphase, phase);
}
...
void rtpghi_update_plan::execute(const double *s,
                                           const double *tgrad,
                                           const double *fgrad,
                                           const double *startphase,
                                           double *phase)
...
void rtpghi_update_plan::execute_common(const double *s,
                                        const double *tgrad,
                                        const double *fgrad,
                                        const double *startphase,
                                        double *phase)
{
    ...
    rtpghi_heap_reset(h, s);

    for (int m = 0; m < M2; ++m) {
        if (donemask[m] > 0) {
            // We already know this one
            rtpghi_heap_insert(h, m + M2);
            quickbreak -= 1;
        }
        else {
            if (slog2[m] <= logabstol) {
                ...
                rtpghi_heap_insert(h, m);
            }
        }
    }

    int w = -1;
    while ((quickbreak > 0) && (w = rtpghi_heap_delete(h)) >= 0) {
        if (w >= M2) {
            // Next frame
            int wprev = w - M2;
...
double rtpghi_update_plan::random_phase()
{
    return _dist(_rand);
}
...
static void rtpghi_fgrad(const double *phase, int M, double stretch, double *fgrad)
{
    ...
    for (int m = 1; m < M2 - 1; ++m) {
        fgrad[m] = (princarg(phase[m + 1] - phase[m]) +
                    princarg(phase[m]     - phase[m - 1])) / (2.0) * stretch;
    }
...
{
    for (int l = 0; l < L; ++l) {
        c[l].real(s[l] * cos(phase[l]));
        c[l].imag(s[l] * sin(phase[l]));
    }
}
